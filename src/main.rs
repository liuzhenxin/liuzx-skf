...
/// Alias resolution for the migrated domain handlers.
///
/// The default alias returns the provider the server already built; every other
/// configured alias keeps its own library, loaded once and cached by `get_api`,
/// re-expressed through the provider trait so the handler code has one shape.
impl ServerContext for SkfContext {
    fn default_alias(&self) -> &str {
        &self.config.default
    }

    fn resolve(&self, alias: &str) -> Result<Arc<dyn SkfProvider>, String> {
        if alias == self.config.default {
            return Ok(Arc::clone(&self.provider));
        }
        let api = self.get_api(alias).map_err(|e| e.to_string())?;
        Ok(Arc::new(NativeSkfProvider::from_api(alias, api)))
    }
}
...
fn handle_request(
    ...
    match req.method.as_str() {
        ...
        "DigestInit" => {
            ...
            // alive without a per-call load. The device is opened per digest and
